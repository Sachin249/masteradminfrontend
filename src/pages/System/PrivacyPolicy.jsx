...
console.log(isDataComing)
    const getData = async () => {
        try {
          setisLoading(true)
            const res=await axios.get(`${REACT_APP_API_PORT}api/admin/privacypolicy/list`,
            {
              headers: {
            ...
            console.log('sac')
             console.log(res.data.data)
              ...
              if(res.data.data !== null){
                ...
                settextValue(res.data.data.privacy_policy)
                setInitialContent(res.data.data.privacy_policy)
                setInitialContent1(res.data.data.term_and_condition)
                settextValue1(res.data.data.term_and_condition)
    ...
    const handleSubmit = async (e) => {
        ...
        formdata.append("privacy_policy", textValue);
        formdata.append("term_and_condition", textValue1);
        setapiIsLoading(true);
        await axios
          .post(`${REACT_APP_API_PORT}api/admin/privacypolicy/create`, formdata, {
            headers: {
              "Content-Type": "application/json",
              "x-access-token": admin?.jwtoken,
            },
      ...
      const handleUpdate = async (e) => {
        e.preventDefault();
        const formdata = new FormData();
        formdata.append("privacyId",updateId);
        formdata.append("privacy_policy", textValue);
        formdata.append("term_and_condition", textValue1);
        setapiIsLoading(true);
        await axios
          .post(`${REACT_APP_API_PORT}api/admin/privacypolicy/update`, formdata, {
            headers: {
              "Content-Type": "application/json",
              "x-access-token": admin?.jwtoken,
            },
