  ...
  const getData = async () => {
    ...
    try {
      const res = await axios.get(
        `${REACT_APP_API_PORT}api/admin/profile/getprofile`,
        {
          headers: {
...
// change password api 
  ...
  const handleUpdatePassword = async (e) => {
    ...
    //   props.showAlert("Current Password is required", "red");
    //   return false;
    // }
    // if (new_password === "") {
    //   props.setAlertBox(true);
    //   props.showAlert("New Password is required", "red");
    //   return false;
    // }
    // if (confirm_password === "") {
    //   props.setAlertBox(true);
    //   props.showAlert("Confirm Passsword is required", "red");
    //   return false;
    // }
    ...
    try {
      ...
      let result = await fetch(
        `${REACT_APP_API_PORT}api/admin/profile/changepassword`,
        {
          headers: {
            "Content-Type": "application/json",
            "x-access-token": admin.jwtoken,
      ...
      if (result.success) {
        ...
        props.setAlertBox(true);
        props.showAlert(result.success, "green");
      }
      if (result.error) {
        // toast.error('Incorrect Old Password')
        props.setAlertBox(true);
        props.showAlert(result.error, "red");
      }
      if (result.errors) {
      ...
      if (error.response.request.status === 500) {
        ...
        // toast.error(error.response.data.message)
        props.setAlertBox(true);
        props.showAlert(response.data.message, "red");
        navigate("/");
      }
    }
  };
...
// update profile api
  ...
  const handleUpdateProfile = async (e) => {
    ...
    if (name === "") {
      props.setAlertBox(true);
      props.showAlert("Name is required", "red");
      return false;
    }
    if (address === "") {
      props.setAlertBox(true);
      props.showAlert("Address is required", "red");
      return false;
    }
    if (phone === "") {
      props.setAlertBox(true);
      props.showAlert("Number is required", "red");
      return false;
    }
    ...
    setapiIsLoading(true);
    await axios
      .post(`${REACT_APP_API_PORT}api/admin/profile/updateprofile`, formdata, {
        headers: {
      ...
      .catch((error) => {
        ...
        if (error.response.request.status === 500) {
          props.setAlertBox(true);
          props.showAlert("server error", "red");
          localStorage.clear();
          // toast.error(error.response.data.message)
          navigate("/");
        }
  ...
  const handlePhone = (e) => {
    ...
    if (e.target.value.length > 10 || e.target.value.length < 10) {
      setMessage1({ mess: "Mobile no length must be 10" });
      var mobile = document.getElementById("number");
      mobile.style.border = "1px solid red";
    } else {
      setMessage1({});
      var mobile = document.getElementById("number");
      mobile.style.border = "1px solid #a19e9e";
    }
  };
                    ...
                    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                      ...
                      <div className="p-7">
                        <form>
                          ...
                          <span className="text-[#dd0e0e] text-[15px] relative top-[-12px] left-[2px]">
                          {message1.mess}
                        </span>
                      ...
                      <div className="p-7">
                        ...
                        >
                          ...
                          <div className="flex flex-col items-center justify-center space-y-3">
                            ...
                            <p>(max, 800 X 800px)</p> */}
                            {(image) ?
                            <img src={image?preview:userThree} alt="User"  className="w-48 h-48 rounded-full"/>
                        : <img src={`${REACT_APP_API_PORT}uploads/${preview}`}  alt="User"  className="w-48 h-48 rounded-full"/>
                            }
                  ...
                  <div className="flex flex-wrap items-center">
                    <div className="hidden w-full border-stroke dark:border-strokedark xl:block xl:w-1/2 xl:border-r-2">
                      ...
                      <img
                        ...
                        src="/assets/logo-162ee3ec.svg"
                        alt="Logo"
                      />
                      <img
                        className="dark:hidden"
                        src="/assets/logo-dark-893f6bd0.svg"
                        alt="Logo"
                      />
                    </a>
